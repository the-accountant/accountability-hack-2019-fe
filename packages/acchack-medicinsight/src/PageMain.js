...
<<<<<<< HEAD
  ...
  _updateDomain(event) {
    ...
    if (this.domainsSelected.length > 0) {
      const _isSelectedDomain = (value) => {
        ...
        return this.domainsSelected.indexOf(value.Domein) !== -1
      }
  ...
  render() {
    return html `
    <checkbox-selector .values="${this.domains}" .selected="${this.domainsSelected}" title="Domain" @update="${this._updateDomain}"></checkbox-selector>
    <checkbox-selector .values="${this.company}" .selected="${this.companySelected}" title="Company" @update="${this._updateCompany}"></checkbox-selector>

    <!-- Magic here with domains and company -->
  ${this.dataView.map((item, index) => html `<p>${index+1}. ${item.Stofnaam}</p>`)}
=======
    super();
